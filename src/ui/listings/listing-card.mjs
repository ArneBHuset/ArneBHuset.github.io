...
export async function listingCardBuild() {
  try {
    ...
    if (document.body.dataset.page === 'profile') {
      profileListingInteraction = `  <div class="flex absolute bottom-2 right-3 ">
      <span id="deleteButton" class="focus:outline-none cursor-pointer" tabindex="0" role="button" aria-label="delete z-9999">
      <span class="material-symbols-outlined text-black bg-secondary1 rounded-full p-1 hover:outline-black">
          delete
      </span>
  </span>
  <span id="updateButton" class="focus:outline-none ml-4 cursor-pointer" tabindex="0" role="button" aria-label="edit"> <!-- Added 'ml-4' for spacing -->
      <span class="material-symbols-outlined text-black bg-secondary1 rounded-full p-1 hover:shadow-md z-10">
          edit
      </span>
  </span>
